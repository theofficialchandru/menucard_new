...
def menu_list(request):
    if request.user.is_hotel:
        ...
        # Get counts
        available_menu_items_count = available_menu_items.count()
        finished_menu_items_count = finished_menu_items.count()
...
def menu_items_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        # Retrieve the Category instance corresponding to the provided category ID
        category = Category.objects.get(pk=category_id)
        
        if image:
            menu_item.image = image
        menu_item.name = name
        menu_item.category = category  # Assign the Category instance, not the ID
        menu_item.price = price
        menu_item.description = description
        menu_item.save()
        return redirect('hotel_index')

    return render(request, 'hotel/edit_menu_items.html', {'menu_item': menu_item})

# @login_required
# def view_menu(request):
#     menu_items = MenuItem.objects.filter(user=request.user)
#     # Render the menu items for the logged-in user
